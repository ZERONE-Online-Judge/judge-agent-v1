...
# Sets testcase parallelism to 1; keeps submission slots at 1, image, CPU and RAM.
...
assert settings.total_slots == 1, "제출 슬롯이 1이 아닙니다. 비교 조건을 확인하세요."
assert settings.testcase_parallelism in (1, 4), "테스트 병렬 수가 예상과 다릅니다."
...
backup="/var/backups/zoj-testcase-parallelism/$stamp"
mkdir -p "$backup"
chmod 700 "$backup"
...
lines=[line for line in p.read_text().splitlines() if not re.match(r'^\s*(?:export\s+)?JUDGE_TESTCASE_PARALLELISM\s*=', line)]
lines.append('JUDGE_TESTCASE_PARALLELISM=1')
fd,name=tempfile.mkstemp(prefix='.testcase-parallelism-',dir=p.parent)
try:
...
assert settings.testcase_parallelism == 1, "테스트 병렬 설정이 Compose에서 덮어써졌습니다."
print("확인:", settings.node_name, "slots=1 testcase_parallelism=1 version="+settings.agent_version)
'
trap - ERR
...
if [[ ${#failed[@]} -gt 0 ]]; then
  printf '미완료 노드: %s. 모두 slots=1 testcase_parallelism=1로 확인되기 전에는 비교 실험을 시작하지 마세요.\n' "${failed[*]}" >&2
  exit 1
fi
printf '\n선택 노드의 설정 변경 완료. 각 노드의 확인 출력에서 slots=1 testcase_parallelism=1을 확인한 뒤 실험하세요.\n'
